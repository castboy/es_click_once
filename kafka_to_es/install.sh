...
function log() {
	if [ -z $2 ]
	then
		echo "$(date '+%G-%m-%d %H:%M:%S'): INF   detail: $1"  >> $LOG
	else
		echo "$(date '+%G-%m-%d %H:%M:%S'): CRT   detail: $1" >> $LOG 
		exit 1
	fi
}
...
function init_vars() {
	LOG="$(pwd)/install-$(date '+%G-%m-%d_%H:%M:%S').log"
...
function conf_file() {
	...
	kafka_brokers=192.168.1.103,192.168.1.102
	
	es_nodes=192.168.1.103,192.168.1.102
	es_port=9200
